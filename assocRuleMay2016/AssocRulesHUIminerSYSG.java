 ...
 *
 */
public class AssocRulesHUIminerSYSG {
		...
		public Rules runAlgorithm(String input, String output, double minconf) throws IOException {
			...
			HUIset = new HashSet<String>();//new HashSet<String>();
			...
			try {
				...
				while ((thisLine = myInput.readLine()) != null) {
					...
					// added function DROP ZERO Utility Item
					//int index = 0;
...
/*						int util = Integer.parseInt(utilityArray[i]);
						if(util > 0){
							itemsArray[index] = Integer.parseInt(items[i]);
							itemsetUtilityArray[index] = Integer.parseInt(utilityArray[i]);
							index++;
						}*/
			...
			checkMemory();
			// start the algorithm
			return runAlgorithm(patterns, HUIset, output);
		 ...
		 */
		private Rules runAlgorithm(List<UItemset> patterns, Set<String> HUIset,String output)
				throws IOException {
			...
			*/
			...
			rulesCount = 0;
			Combination subsetAgent = Combination.getInstance();
			// save itemsets in a member variable
			this.patterns = patterns;
			//System.out.println(HUIset.contains("18338703"));
			// loop through the patterns to discover rules
			for(int i = 0; i < patterns.size(); i++){
				Rules rulesInPattern =  new Rules("RULES PER PATTERN", minconf); // added for topK
				...
				eachItemset = eachItemset.sort(); // sort the UItemset in ascending order
				...
				for(UItemset consequentY: eachItemset.getUItemsetList()){
					...
					if(HUIset.contains(consequentY.getUItemsetString())){
						...
						List<List<UItemset>> subsetListX = eachItemset.cloneItemSetMinusAnItemset(consequentY).getSubset(subsetAgent);
						subsetAgent.clearSet();
/*						System.out.println("=>" + eachItemset.getUItemsetString());
						for(UItemset e: subsetListX){
							System.out.println("* *" + e.getUItemsetString());
						}
						System.out.println("-------");*/
						for(List<UItemset> list: subsetListX){
							for(UItemset antecedentX: list){
...
/*									System.out.println(aRule.toString());
									...
									writer.newLine();*/
									//rules.addRule(aRule);
										if(!rulesInPattern.contains(aRule)){
											//System.out.println(aRule.toString());
											rulesInPattern.addRule(aRule);
										}
