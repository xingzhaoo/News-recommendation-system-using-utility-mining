...
public class AssocRulesTopics {
	...
	// for statistics
	protected final static double EPSILON = 0.0000001; // precision for double comparison
	...
	public void topicRuleGen(String topic1, String topic2){
		...
		for(String nw: newsRules.keySet()){
			...
			for(TRule r : nwRules){ // r is news rules which uses TRule for storage
				...
				//System.out.println("236:" + r.getTopic1() + ":" + singleItemProb.get(r.getTopic1()));
				// get P(nw')
				double p_singleNW = singleItemProb.get(r.getTopic1());
				// P(nw|NW) = sum(P(nw|nw')P(nw'))
				// p_nw += r.getProbConf() * p_singleNW;
				p_nw += doubleMultiply(r.getProbConf(), p_singleNW);
				//System.out.println(nw+":"+p_singleNW+":"+ p_nw); 
			}
			...
			numerator += doubleMultiply(doubleMultiply(p_nw, p_t2_nw), p_t1_nw);
			
			//System.out.println(numerator+"/"+denominator); 
		}
		
		if (denominator - EPSILON > 0){
	...
	/**
	 * alternative to the small double value multiplication to avoid
	 * overflow 
	 * @param a, 0 <= a <= 1
	 * @param b, 0 <= b <= 1
	 * @return a*b
	 ...
	 * @param newsRule 	news rules 
	 */
	public void loadNewsRules(Rules newsRule){
		newsRules = new HashMap<String, List<TRule>>();
		for(Rule r: newsRule.getRules()){			
			//split the transaction according to the separator
			String split[] = r.toString().split("#Rule Utility:");
...
/*		rulesCount = rules.size();
		if(writer != null){				
			...
			StringBuilder buffer = new StringBuilder();
			for(TRule r: rules){
				buffer.append(r.getTopic1());
				buffer.append("==>");
				buffer.append(r.getTopic2());
				buffer.append("#tconf:");ja
				buffer.append(r.getProbConf());
				buffer.append("\n");								
			}
			writer.write(buffer.toString());	
		}
	...
	public void printStats() {
		System.out.println("=============  Association Topic Rule ALGORITHM - STATS =============");
