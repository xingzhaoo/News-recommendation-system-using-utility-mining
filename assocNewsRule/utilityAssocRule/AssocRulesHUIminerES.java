 ...
 *
 */
public class AssocRulesHUIminerES {
		...
		protected Rules rules; // 1-1 news rules for topic rules generation
		// added for news rules which combining two systems: 1-1 news rules support topic rules and many-1 news rules 
		protected Rules nwRules; // store many-1 news rules for news recommendation
		...
		private Rules runAlgorithm(List<UItemset> patterns, String output)
			...
			*/
			writer = new BufferedWriter(new FileWriter(output)); 
			rules =  new Rules("HUI MINED ASSOCIATION RULES", minconf);  // 1-1 rules
			...
			for(int i = 0; i < patterns.size(); i++){
				...
				ExhaustAnalysis evaluation = new ExhaustAnalysis(HUIset, minconf); // 1-1 news rules
				...
				ExhaustAnalysis2NR evaluation2NR = new ExhaustAnalysis2NR(HUIset, minconf); // many - 1 news rules
			...
			nwRules.sortByConfidence();
			// save to file
			saveSortedRule(nwRules); // text presentation
		...
		public void printStats() {
			System.out.println("=============  Association News Rule - ES ALGORITHM - STATS =============");
